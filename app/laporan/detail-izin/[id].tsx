...
export default function DetailIzinScreen() {
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${getApiUrl(API_CONFIG.ENDPOINTS.DETAIL_LAPORAN)}?type=izin&id=${id}`);
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.headerContent}>
          <View style={styles.headerLeft}>
            ...
            <View style={styles.headerTitleContainer}>
              ...
              <Text style={styles.headerTitle}>Detail Izin/Cuti</Text>
            </View>
          </View>
        </View>
      </View>
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Informasi Izin/Cuti</Text>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 60, height: 60, borderRadius: 30, backgroundColor: '#FFF3E0', justifyContent: 'center', alignItems: 'center', marginRight: 12, overflow: 'hidden' },
  avatarImage: { width: 60, height: 60, borderRadius: 30 },
  avatarText: { color: '#FF9800', fontWeight: 'bold', fontSize: 24 },
  profileInfo: { flex: 1 },
